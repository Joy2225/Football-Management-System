...
char ejected_players[2][11][3];                         //array which contains the players who have received a red card
int rp1=0,rp2=0;                                        //rp1 for number of players who got red card of 1st team, rp2 for number of players who got red card for 2nd team
char teamtA[60];                                        //stores the team name of 1st Team
char teamtB[60];                                        //stores the team name of 2nd Team
int goal1=0,goal2=0;

char attribute[5][20]={"Player_Name", "Jersey_Number", "Goals", "Yellow_Card", "Red_Card"};     //column headings

FILE *team_A; FILE *team_B;               //pointer variables for file extraction from Excel
                   
void team_initialize(){
    printf("Enter the name of the 1st Team: ");
    scanf(" %[^\n]",team_name[0]);                  //team1 name input
    strcpy(teamtA,team_name[0]);                    //copying team name to teamtA
    printf("Enter the name of the 2nd Team: ");
    scanf(" %[^\n]",team_name[1]);                 //team2 name input
...
void initialize(char n[2][22][5][60]){     //initializing the array
    ...
    strcpy(n[0][21][2]," ");            //to initialize null values to coach in column goals
    strcpy(n[1][21][2]," ");
}
...
int jersey_number(int n){                       //selecting a particular player for further functions
    ...
    do{
        ...
        printf("\n");
        for(int i=1;i<12;i++){
            if(atoi(Names[n][i][1])==j_no){                 //"atoi"-converts the string to integer in the array 
